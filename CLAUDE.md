...
Don't get tunnel-visioned on one error - a mod initialization failure earlier in the log can cause cascading errors that appear unrelated. In particular, `Cannot get config value before config is loaded.` is *never* the root cause—it's always a symptom of an earlier mod initialization failure.
