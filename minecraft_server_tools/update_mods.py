...
def get_jar_name_for_curseforge_file(curseforge_file):  #557 (line in Coconut source)
    return (((get_curseforge_download_url(curseforge_file)).rsplit("/", 1))[-1])  #557 (line in Coconut source)



def correct_modloader(versions, jar_name):  #564 (line in Coconut source)
    versions = [v.lower() for v in versions]  #565 (line in Coconut source)

    if MODLOADER.lower() in versions:  #567 (line in Coconut source)
        return True  #568 (line in Coconut source)
    if any((wrong_modloader.lower() in versions for wrong_modloader in WRONG_MODLOADERS)):  #569 (line in Coconut source)
        return False  #570 (line in Coconut source)

    jar_name = jar_name.lower()  #572 (line in Coconut source)
    if has_bad_modloader(jar_name):  #573 (line in Coconut source)
        return False  #574 (line in Coconut source)

    return True  #576 (line in Coconut source)



def find_curseforge_file_for_jar(curseforge_files, find_jar_name):  #579 (line in Coconut source)
    for file_data in curseforge_files:  #580 (line in Coconut source)
        jar_name = get_jar_name_for_curseforge_file(file_data)  #581 (line in Coconut source)
        if jar_name is not None and are_same_jar(jar_name, find_jar_name):  #582 (line in Coconut source)
            return file_data  #583 (line in Coconut source)
    return None  #584 (line in Coconut source)



@_coconut_tco  #587 (line in Coconut source)
def get_latest_version(mod_name, curseforge_id, old_jar_name):  #587 (line in Coconut source)
    curseforge_files = get_curseforge_files(curseforge_id)  #588 (line in Coconut source)

    old_curseforge_file = find_curseforge_file_for_jar(curseforge_files, old_jar_name)  #590 (line in Coconut source)
    if old_curseforge_file is None:  #591 (line in Coconut source)
        print("WARNING: Could not find curseforge file for existing jar: {_coconut_format_0}".format(_coconut_format_0=(old_jar_name)))  #592 (line in Coconut source)
    old_file_time = get_curseforge_file_time(old_curseforge_file, mod_name) if old_curseforge_file is not None else BEGINNING_OF_TIME  #593 (line in Coconut source)

    curseforge_files_and_versions = []  #595 (line in Coconut source)
    for file_data in curseforge_files:  #596 (line in Coconut source)
        versions = file_data["gameVersions"]  #597 (line in Coconut source)
        jar_name = get_jar_name_for_curseforge_file(file_data)  #598 (line in Coconut source)
        if (jar_name is not None and jar_name.endswith(".jar") and correct_modloader(versions, jar_name) and get_curseforge_file_time(file_data, mod_name) >= old_file_time):  #599 (line in Coconut source)
            curseforge_files_and_versions.append((file_data, versions))  #605 (line in Coconut source)

    correctly_versioned_files = []  #607 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #608 (line in Coconut source)
        if ver_join(MC_VERSION) in versions:  #609 (line in Coconut source)
            correctly_versioned_files.append(file_data)  #610 (line in Coconut source)
    if correctly_versioned_files:  #611 (line in Coconut source)
        return _coconut_tail_call(best_release, correctly_versioned_files, mod_name)  #612 (line in Coconut source)
    print("No correctly versioned files found for mod {_coconut_format_0!r}.".format(_coconut_format_0=(mod_name)))  #613 (line in Coconut source)

    compatibly_versioned_files = []  #615 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #616 (line in Coconut source)
        for raw_ver in versions:  #617 (line in Coconut source)
            try:  #618 (line in Coconut source)
                ver = ver_split(raw_ver)  #619 (line in Coconut source)
            except ValueError:  #620 (line in Coconut source)
                pass  #621 (line in Coconut source)
            else:  #622 (line in Coconut source)
                if MC_VERSION[:2] <= ver <= MC_VERSION:  #623 (line in Coconut source)
                    compatibly_versioned_files.append(file_data)  #624 (line in Coconut source)
    if compatibly_versioned_files:  #625 (line in Coconut source)
        return _coconut_tail_call(best_release, compatibly_versioned_files, mod_name)  #626 (line in Coconut source)

    maybe_compatible_files = []  #628 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #629 (line in Coconut source)
        for ver in versions:  #630 (line in Coconut source)
            if ver.startswith(ver_join(MC_VERSION[:2])):  #631 (line in Coconut source)
                maybe_compatible_files.append(file_data)  #632 (line in Coconut source)
                break  #633 (line in Coconut source)
    if maybe_compatible_files:  #634 (line in Coconut source)
        return _coconut_tail_call(best_release, maybe_compatible_files, mod_name)  #635 (line in Coconut source)
    print("No compatibly versioned files found for mod {_coconut_format_0!r} in:".format(_coconut_format_0=(mod_name)))  #636 (line in Coconut source)
    pprint(list(timestamp_sort(curseforge_files))[:MAX_DEBUG_RESULTS])  #637 (line in Coconut source)



def are_same_jar(jar_name_1, jar_name_2):  #640 (line in Coconut source)
    return jar_name_1.replace(" ", "+") == jar_name_2.replace(" ", "+")  #641 (line in Coconut source)



def get_mod_names_to_latest_versions(mod_names_to_curseforge_ids, mod_names_to_jar_names):  #644 (line in Coconut source)
    mod_names_to_latest_versions = _coconut.dict()  #645 (line in Coconut source)
    missing_mods = []  #646 (line in Coconut source)
    for mod_name, curseforge_id in mod_names_to_curseforge_ids.items():  #647 (line in Coconut source)
        jar_name = mod_names_to_jar_names[mod_name]  #648 (line in Coconut source)
        latest_version = get_latest_version(mod_name, curseforge_id, jar_name)  #649 (line in Coconut source)
        if latest_version is None:  #650 (line in Coconut source)
            missing_mods.append(mod_name)  #651 (line in Coconut source)
        else:  #652 (line in Coconut source)
            mod_names_to_latest_versions[mod_name] = latest_version  #653 (line in Coconut source)
    return mod_names_to_latest_versions, missing_mods  #654 (line in Coconut source)



def get_updated_mod_names_to_files(mod_names_to_jar_names, mod_names_to_latest_versions):  #657 (line in Coconut source)
    updated_mod_names_to_files = _coconut.dict()  #658 (line in Coconut source)
    for mod_name, latest_file in mod_names_to_latest_versions.items():  #659 (line in Coconut source)
        old_jar = mod_names_to_jar_names[mod_name]  #660 (line in Coconut source)
        new_jar = get_jar_name_for_curseforge_file(latest_file)  #661 (line in Coconut source)
        if new_jar is not None and not are_same_jar(new_jar, old_jar):  #662 (line in Coconut source)
            updated_mod_names_to_files[mod_name] = latest_file  #663 (line in Coconut source)
    return updated_mod_names_to_files  #664 (line in Coconut source)



def download_file(curseforge_file, updated_mods_dir, mod_name):  #667 (line in Coconut source)
    jar_name = get_jar_name_for_curseforge_file(curseforge_file)  #668 (line in Coconut source)
    assert jar_name is not None, "cannot download from curseforge file: {_coconut_format_0!r}".format(_coconut_format_0=(curseforge_file))  #669 (line in Coconut source)
    url = get_curseforge_download_url(curseforge_file)  #670 (line in Coconut source)
    new_jar_path = os.path.join(updated_mods_dir, jar_name)  #671 (line in Coconut source)
    if os.path.exists(new_jar_path):  #672 (line in Coconut source)
        print("WARNING: attempting to redownload existing jar {_coconut_format_0!r}".format(_coconut_format_0=(jar_name)))  #673 (line in Coconut source)
    else:  #674 (line in Coconut source)
        print("Downloading {_coconut_format_0}...".format(_coconut_format_0=(jar_name)))  #675 (line in Coconut source)
        new_mod_name = get_mod_name(jar_name, silent=True)  #676 (line in Coconut source)
        if new_mod_name != mod_name:  #677 (line in Coconut source)
            print("\tWARNING: new mod name: {_coconut_format_0!r} -> {_coconut_format_1!r}".format(_coconut_format_0=(mod_name), _coconut_format_1=(new_mod_name)))  #678 (line in Coconut source)
        result = requests.get(url)  #679 (line in Coconut source)
        with open(new_jar_path, "wb") as jar_fobj:  #680 (line in Coconut source)
            jar_fobj.write(result.content)  #681 (line in Coconut source)



def update_files(updated_mod_names_to_files, updated_mods_dir):  #684 (line in Coconut source)
    seen_jar_names = _coconut.dict()  #685 (line in Coconut source)
    for mod_name, curseforge_file in updated_mod_names_to_files.items():  #686 (line in Coconut source)
...
def move_old_files(updated_mod_names_to_files, mod_names_to_jar_names, mods_dir, old_mods_dir):  #696 (line in Coconut source)
    for mod_name in updated_mod_names_to_files:  #697 (line in Coconut source)
        jar_name = mod_names_to_jar_names[mod_name]  #698 (line in Coconut source)
        current_jar_path = os.path.join(mods_dir, jar_name)  #699 (line in Coconut source)
        new_jar_path = os.path.join(old_mods_dir, jar_name)  #700 (line in Coconut source)
        os.rename(current_jar_path, new_jar_path)  #701 (line in Coconut source)



def make_dirs(*dirs):  #704 (line in Coconut source)
    for d in dirs:  #705 (line in Coconut source)
        if not os.path.exists(d):  #706 (line in Coconut source)
            os.mkdir(d)  #707 (line in Coconut source)



def update_mods(mods_dir, updated_mods_dir, old_mods_dir, dry_run=False, interact=None, google=False):  #710 (line in Coconut source)
    if interact is None and not PRINT_DEBUG:  #711 (line in Coconut source)
        interact = False  #712 (line in Coconut source)
    try:  #713 (line in Coconut source)
        mod_names_to_jar_names = get_mod_names_to_jar_names(mods_dir)  #714 (line in Coconut source)
        mod_names_to_curseforge_names, nulled_mods = get_curseforge_names_for(mod_names_to_jar_names, google=google)  #715 (line in Coconut source)
        if not dry_run:  #716 (line in Coconut source)
            mod_names_to_curseforge_ids, missing_ids_mods = get_mod_names_to_curseforge_ids(mod_names_to_curseforge_names)  #717 (line in Coconut source)
            mod_names_to_latest_versions, missing_files_mods = get_mod_names_to_latest_versions(mod_names_to_curseforge_ids, mod_names_to_jar_names)  #718 (line in Coconut source)
            updated_mod_names_to_files = get_updated_mod_names_to_files(mod_names_to_jar_names, mod_names_to_latest_versions)  #719 (line in Coconut source)
            if updated_mod_names_to_files:  #720 (line in Coconut source)
                make_dirs(updated_mods_dir, old_mods_dir)  #721 (line in Coconut source)
                update_files(updated_mod_names_to_files, updated_mods_dir)  #722 (line in Coconut source)
                move_old_files(updated_mod_names_to_files, mod_names_to_jar_names, mods_dir, old_mods_dir)  #723 (line in Coconut source)
        else:  #724 (line in Coconut source)
            missing_ids_mods = []  #725 (line in Coconut source)
            missing_files_mods = []  #726 (line in Coconut source)
        return nulled_mods + missing_ids_mods + missing_files_mods  #727 (line in Coconut source)
    except Exception:  #728 (line in Coconut source)
        if interact is not False:  #729 (line in Coconut source)
            traceback.print_exc()  #730 (line in Coconut source)

            from coconut import embed  #732 (line in Coconut source)
            embed()  #733 (line in Coconut source)
        raise  #734 (line in Coconut source)
    if interact:  #735 (line in Coconut source)
        from coconut import embed  #736 (line in Coconut source)
        embed()  #737 (line in Coconut source)



def update_all(mods_dirs, dry_run=False, interact=None, google=False):  #740 (line in Coconut source)
    couldnt_update = []  #741 (line in Coconut source)
    for mods_dir in mods_dirs:  #742 (line in Coconut source)
        updated_mods_dir = mods_dir + UPDATED_MODS_DIR_SUFFIX  #743 (line in Coconut source)
        old_mods_dir = mods_dir + OLD_MODS_DIR_SUFFIX  #744 (line in Coconut source)
        couldnt_update += update_mods(mods_dir, updated_mods_dir, old_mods_dir, dry_run=dry_run, interact=interact, google=google)  #745 (line in Coconut source)
    for mod_name in couldnt_update:  #746 (line in Coconut source)
        print("Unable to automatically update mod: {_coconut_format_0}".format(_coconut_format_0=(mod_name)))  #747 (line in Coconut source)



@_coconut_tco  #750 (line in Coconut source)
def parse_args():  #750 (line in Coconut source)
    parser = argparse.ArgumentParser(description="Update mods from CurseForge to their latest versions.")  #751 (line in Coconut source)
    parser.add_argument("--dry-run", action="store_true", help="Only check for missing CurseForge name mappings without downloading updates")  #754 (line in Coconut source)
    parser.add_argument("--google", action="store_true", help="Use Google search to find CurseForge names for unknown mods")  #759 (line in Coconut source)
    return _coconut_tail_call(parser.parse_args)  #764 (line in Coconut source)



def main():  #767 (line in Coconut source)
    args = parse_args()  #768 (line in Coconut source)

    sync_mods.main()  #770 (line in Coconut source)

    update_all(UPDATE_MODS_DIRS, dry_run=args.dry_run, google=args.google)  #772 (line in Coconut source)



if __name__ == "__main__":  #779 (line in Coconut source)
    main()  #780 (line in Coconut source)
