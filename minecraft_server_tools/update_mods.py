...
def get_latest_version(mod_name, curseforge_id, old_jar_name):  #585 (line in Coconut source)
    ...
    for file_data in curseforge_files:  #594 (line in Coconut source)
        ...
        if (jar_name is not None and jar_name.endswith(".jar") and correct_modloader(versions, jar_name) and get_curseforge_file_time(file_data, mod_name) >= old_file_time):  #597 (line in Coconut source)
            curseforge_files_and_versions.append((file_data, versions))  #603 (line in Coconut source)

    correctly_versioned_files = []  #605 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #606 (line in Coconut source)
        if ver_join(MC_VERSION) in versions:  #607 (line in Coconut source)
            correctly_versioned_files.append(file_data)  #608 (line in Coconut source)
    if correctly_versioned_files:  #609 (line in Coconut source)
        return _coconut_tail_call(best_release, correctly_versioned_files, mod_name)  #610 (line in Coconut source)
    print("No correctly versioned files found for mod {_coconut_format_0!r}.".format(_coconut_format_0=(mod_name)))  #611 (line in Coconut source)

    compatibly_versioned_files = []  #613 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #614 (line in Coconut source)
        for raw_ver in versions:  #615 (line in Coconut source)
            try:  #616 (line in Coconut source)
                ver = ver_split(raw_ver)  #617 (line in Coconut source)
            except ValueError:  #618 (line in Coconut source)
                pass  #619 (line in Coconut source)
            else:  #620 (line in Coconut source)
                if MC_VERSION[:2] <= ver <= MC_VERSION:  #621 (line in Coconut source)
                    compatibly_versioned_files.append(file_data)  #622 (line in Coconut source)
    if compatibly_versioned_files:  #623 (line in Coconut source)
        return _coconut_tail_call(best_release, compatibly_versioned_files, mod_name)  #624 (line in Coconut source)

    maybe_compatible_files = []  #626 (line in Coconut source)
    for file_data, versions in curseforge_files_and_versions:  #627 (line in Coconut source)
        for ver in versions:  #628 (line in Coconut source)
            if ver.startswith(ver_join(MC_VERSION[:2])):  #629 (line in Coconut source)
                maybe_compatible_files.append(file_data)  #630 (line in Coconut source)
                break  #631 (line in Coconut source)
    if maybe_compatible_files:  #632 (line in Coconut source)
        return _coconut_tail_call(best_release, maybe_compatible_files, mod_name)  #633 (line in Coconut source)
    print("No compatibly versioned files found for mod {_coconut_format_0!r} in:".format(_coconut_format_0=(mod_name)))  #634 (line in Coconut source)
    pprint(list(timestamp_sort(curseforge_files))[:MAX_DEBUG_RESULTS])  #635 (line in Coconut source)



def are_same_jar(jar_name_1, jar_name_2):  #638 (line in Coconut source)
    return jar_name_1.replace(" ", "+") == jar_name_2.replace(" ", "+")  #639 (line in Coconut source)



def get_mod_names_to_latest_versions(mod_names_to_curseforge_ids, mod_names_to_jar_names):  #642 (line in Coconut source)
    mod_names_to_latest_versions = _coconut.dict()  #643 (line in Coconut source)
    missing_mods = []  #644 (line in Coconut source)
    for mod_name, curseforge_id in mod_names_to_curseforge_ids.items():  #645 (line in Coconut source)
        jar_name = mod_names_to_jar_names[mod_name]  #646 (line in Coconut source)
        latest_version = get_latest_version(mod_name, curseforge_id, jar_name)  #647 (line in Coconut source)
        if latest_version is None:  #648 (line in Coconut source)
            missing_mods.append(mod_name)  #649 (line in Coconut source)
        else:  #650 (line in Coconut source)
            mod_names_to_latest_versions[mod_name] = latest_version  #651 (line in Coconut source)
    return mod_names_to_latest_versions, missing_mods  #652 (line in Coconut source)



def get_updated_mod_names_to_files(mod_names_to_jar_names, mod_names_to_latest_versions):  #655 (line in Coconut source)
    updated_mod_names_to_files = _coconut.dict()  #656 (line in Coconut source)
    for mod_name, latest_file in mod_names_to_latest_versions.items():  #657 (line in Coconut source)
        old_jar = mod_names_to_jar_names[mod_name]  #658 (line in Coconut source)
        new_jar = get_jar_name_for_curseforge_file(latest_file)  #659 (line in Coconut source)
        if new_jar is not None and not are_same_jar(new_jar, old_jar):  #660 (line in Coconut source)
            updated_mod_names_to_files[mod_name] = latest_file  #661 (line in Coconut source)
    return updated_mod_names_to_files  #662 (line in Coconut source)



def download_file(curseforge_file, updated_mods_dir, mod_name):  #665 (line in Coconut source)
    jar_name = get_jar_name_for_curseforge_file(curseforge_file)  #666 (line in Coconut source)
    assert jar_name is not None, "cannot download from curseforge file: {_coconut_format_0!r}".format(_coconut_format_0=(curseforge_file))  #667 (line in Coconut source)
    url = get_curseforge_download_url(curseforge_file)  #668 (line in Coconut source)
    new_jar_path = os.path.join(updated_mods_dir, jar_name)  #669 (line in Coconut source)
    if os.path.exists(new_jar_path):  #670 (line in Coconut source)
        print("WARNING: attempting to redownload existing jar {_coconut_format_0!r}".format(_coconut_format_0=(jar_name)))  #671 (line in Coconut source)
    else:  #672 (line in Coconut source)
        print("Downloading {_coconut_format_0}...".format(_coconut_format_0=(jar_name)))  #673 (line in Coconut source)
        new_mod_name = get_mod_name(jar_name, silent=True)  #674 (line in Coconut source)
        if new_mod_name != mod_name:  #675 (line in Coconut source)
            print("\tWARNING: new mod name: {_coconut_format_0!r} -> {_coconut_format_1!r}".format(_coconut_format_0=(mod_name), _coconut_format_1=(new_mod_name)))  #676 (line in Coconut source)
        result = requests.get(url)  #677 (line in Coconut source)
        with open(new_jar_path, "wb") as jar_fobj:  #678 (line in Coconut source)
            jar_fobj.write(result.content)  #679 (line in Coconut source)



def update_files(updated_mod_names_to_files, updated_mods_dir):  #682 (line in Coconut source)
    seen_jar_names = _coconut.dict()  #683 (line in Coconut source)
    for mod_name, curseforge_file in updated_mod_names_to_files.items():  #684 (line in Coconut source)
        jar_name = get_jar_name_for_curseforge_file(curseforge_file)  #685 (line in Coconut source)
        assert jar_name is not None, "cannot update using curseforge file: {_coconut_format_0!r}".format(_coconut_format_0=(curseforge_file))  #686 (line in Coconut source)
        if jar_name in seen_jar_names:  #687 (line in Coconut source)
            print("\tWARNING: resolved multiple mod names to same jar name {_coconut_format_0!r}: {_coconut_format_1!r} and {_coconut_format_2!r}".format(_coconut_format_0=(jar_name), _coconut_format_1=(seen_jar_names[jar_name]), _coconut_format_2=(mod_name)))  #688 (line in Coconut source)
        else:  #689 (line in Coconut source)
            seen_jar_names[jar_name] = mod_name  #690 (line in Coconut source)
        download_file(curseforge_file, updated_mods_dir, mod_name)  #691 (line in Coconut source)



def move_old_files(updated_mod_names_to_files, mod_names_to_jar_names, mods_dir, old_mods_dir):  #694 (line in Coconut source)
    for mod_name in updated_mod_names_to_files:  #695 (line in Coconut source)
        jar_name = mod_names_to_jar_names[mod_name]  #696 (line in Coconut source)
        current_jar_path = os.path.join(mods_dir, jar_name)  #697 (line in Coconut source)
        new_jar_path = os.path.join(old_mods_dir, jar_name)  #698 (line in Coconut source)
        os.rename(current_jar_path, new_jar_path)  #699 (line in Coconut source)



def make_dirs(*dirs):  #702 (line in Coconut source)
    for d in dirs:  #703 (line in Coconut source)
        if not os.path.exists(d):  #704 (line in Coconut source)
            os.mkdir(d)  #705 (line in Coconut source)



def update_mods(mods_dir, updated_mods_dir, old_mods_dir, dry_run=False, interact=None, google=False):  #708 (line in Coconut source)
    if interact is None and not PRINT_DEBUG:  #709 (line in Coconut source)
        interact = False  #710 (line in Coconut source)
    try:  #711 (line in Coconut source)
        mod_names_to_jar_names = get_mod_names_to_jar_names(mods_dir)  #712 (line in Coconut source)
        mod_names_to_curseforge_names, nulled_mods = get_curseforge_names_for(mod_names_to_jar_names, google=google)  #713 (line in Coconut source)
        if not dry_run:  #714 (line in Coconut source)
            mod_names_to_curseforge_ids, missing_ids_mods = get_mod_names_to_curseforge_ids(mod_names_to_curseforge_names)  #715 (line in Coconut source)
            mod_names_to_latest_versions, missing_files_mods = get_mod_names_to_latest_versions(mod_names_to_curseforge_ids, mod_names_to_jar_names)  #716 (line in Coconut source)
            updated_mod_names_to_files = get_updated_mod_names_to_files(mod_names_to_jar_names, mod_names_to_latest_versions)  #717 (line in Coconut source)
            if updated_mod_names_to_files:  #718 (line in Coconut source)
                make_dirs(updated_mods_dir, old_mods_dir)  #719 (line in Coconut source)
                update_files(updated_mod_names_to_files, updated_mods_dir)  #720 (line in Coconut source)
                move_old_files(updated_mod_names_to_files, mod_names_to_jar_names, mods_dir, old_mods_dir)  #721 (line in Coconut source)
        else:  #722 (line in Coconut source)
            missing_ids_mods = []  #723 (line in Coconut source)
            missing_files_mods = []  #724 (line in Coconut source)
        return nulled_mods + missing_ids_mods + missing_files_mods  #725 (line in Coconut source)
    except Exception:  #726 (line in Coconut source)
        if interact is not False:  #727 (line in Coconut source)
            traceback.print_exc()  #728 (line in Coconut source)

            from coconut import embed  #730 (line in Coconut source)
            embed()  #731 (line in Coconut source)
        raise  #732 (line in Coconut source)
    if interact:  #733 (line in Coconut source)
        from coconut import embed  #734 (line in Coconut source)
        embed()  #735 (line in Coconut source)



def update_all(mods_dirs, dry_run=False, interact=None, google=False):  #738 (line in Coconut source)
    couldnt_update = []  #739 (line in Coconut source)
    for mods_dir in mods_dirs:  #740 (line in Coconut source)
        updated_mods_dir = mods_dir + UPDATED_MODS_DIR_SUFFIX  #741 (line in Coconut source)
        old_mods_dir = mods_dir + OLD_MODS_DIR_SUFFIX  #742 (line in Coconut source)
        couldnt_update += update_mods(mods_dir, updated_mods_dir, old_mods_dir, dry_run=dry_run, interact=interact, google=google)  #743 (line in Coconut source)
    for mod_name in couldnt_update:  #744 (line in Coconut source)
        print("Unable to automatically update mod: {_coconut_format_0}".format(_coconut_format_0=(mod_name)))  #745 (line in Coconut source)



@_coconut_tco  #748 (line in Coconut source)
def parse_args():  #748 (line in Coconut source)
    parser = argparse.ArgumentParser(description="Update mods from CurseForge to their latest versions.")  #749 (line in Coconut source)
    parser.add_argument("--dry-run", action="store_true", help="Only check for missing CurseForge name mappings without downloading updates")  #752 (line in Coconut source)
    parser.add_argument("--google", action="store_true", help="Use Google search to find CurseForge names for unknown mods")  #757 (line in Coconut source)
    return _coconut_tail_call(parser.parse_args)  #762 (line in Coconut source)



def main():  #765 (line in Coconut source)
    args = parse_args()  #766 (line in Coconut source)

    sync_mods.main()  #768 (line in Coconut source)

    update_all(UPDATE_MODS_DIRS, dry_run=args.dry_run, google=args.google)  #770 (line in Coconut source)



if __name__ == "__main__":  #777 (line in Coconut source)
    main()  #778 (line in Coconut source)
