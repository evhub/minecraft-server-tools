...
def get_jvm_args(gc, vm, large_pages=USE_LARGE_PAGES):  #405 (line in Coconut source)
    ...
    if large_pages:  #461 (line in Coconut source)
        args += ["-XX:+UseLargePages", "-XX:LargePageSizeInBytes=2M", "-XX:+UseTransparentHugePages"]  # Obydux  # hilltty-flags, brucethemoose  # hilltty-flags, brucethemoose  #462 (line in Coconut source)

    if vm == "java":  #468 (line in Coconut source)
        args += ["-XX:+UseNUMA", "-XX:+UseVectorCmov", "-XX:MaxNodeLimit=240000", "-XX:NodeLimitFudgeFactor=8000"]  # brucethemoose: 8000  # brucethemoose: 240000  # brucethemoose  # hilltty-flags, brucethemoose: True; mukul1127: False; Obydux: True  #469 (line in Coconut source)
    elif vm == "graal":  #475 (line in Coconut source)
        jdk_graal = "jdk.graal" if GRAAL_VERSION >= 22 else "graal"  #476 (line in Coconut source)
        args += ["-XX:+EagerJVMCI", "-D{_coconut_format_0}.TuneInlinerExploration=1".format(_coconut_format_0=(jdk_graal)), "-D{_coconut_format_0}.LoopRotation=true".format(_coconut_format_0=(jdk_graal)), "-D{_coconut_format_0}.CompilerConfiguration=enterprise".format(_coconut_format_0=(jdk_graal))]  # brucethemoose: 1  # brucethemoose  # Obydux  # Obydux  #477 (line in Coconut source)
    ...
    if gc == "G1":  #487 (line in Coconut source)
        args += ["-XX:+UseG1GC", "-XX:AllocatePrefetchStyle=3", "-XX:SurvivorRatio=32", "-XX:InitiatingHeapOccupancyPercent=10", "-XX:MaxTenuringThreshold=1", "-XX:G1ReservePercent=20", "-XX:G1NewSizePercent=28", "-XX:G1HeapRegionSize=16M", "-XX:G1MixedGCCountTarget=3", "-XX:G1RSetUpdatingPauseTimePercent=0", "-XX:G1MaxNewSizePercent=50", "-XX:G1HeapWastePercent=20", "-XX:G1SATBBufferEnqueueingThresholdPercent=30", "-XX:G1ConcMarkStepDurationMillis=5", "-XX:GCTimeRatio=99"]  # brucethemoose, Obydux: 5; default: 10  # brucethemoose: 23; atm: 30; aikar: 40; Obydux: 28; default: 5  # brucethemoose: 10; atm: 15; aikar: 20; Obydux: 10; default: 45  # brucethemoose: 0; atm: 5; default: 10; Obydux: 0  # brucethemoose: 20; atm: 5; default: 5  # brucethemoose: 3; atm: 4; Obydux: 3; default: 8  # atm: 32; brucethemoose: 32; Obydux: 32; default: 8  # brucethemoose, Obydux: 3; default: 1  # brucethemoose, Obydux: 30; default: 60  # brucethemoose: 20; atm: 20; aikar: 15; Obydux: 20  # brucethemoose: 1; atm: 1; Obydux: 1; default: 15  # atm: 40; aikar: 50; default: 60  # brucethemoose: 16M; atm: 8M; aikar: 16M; Obydux: 16M  # brucethemoose: 99; default: 12  #488 (line in Coconut source)
        ...
        args += ["-XX:+UseShenandoahGC", "-XX:AllocatePrefetchStyle=1", "-XX:ShenandoahGCMode=generational", "-XX:ShenandoahGuaranteedGCInterval=1000000"]  # brucethemoose: 1000000  # new  # brucethemoose: 1; default: 1  #511 (line in Coconut source)
