...
def get_jvm_args(gc, vm, large_pages=USE_LARGE_PAGES):  #371 (line in Coconut source)
    ...
    if vm == "java":  #433 (line in Coconut source)
        args += ["-XX:+UseNUMA", "-XX:+UseVectorCmov", "-XX:MaxNodeLimit=240000", "-XX:NodeLimitFudgeFactor=8000"]  # brucethemoose  # hilltty-flags, brucethemoose: True; mukul1127: False  # brucethemoose: 8000  # brucethemoose: 240000  #434 (line in Coconut source)
    ...
    if gc == "G1":  #450 (line in Coconut source)
        ...
        args += ["-XX:+UseShenandoahGC", "-XX:AllocatePrefetchStyle=1", "-XX:ShenandoahGCMode=iu", "-XX:ShenandoahGuaranteedGCInterval=1000000"]  # brucethemoose: 1; default: 1  # hilltty-flags  # brucethemoose: 1000000  #474 (line in Coconut source)
    elif gc == "Z":  #480 (line in Coconut source)
        args += ["-XX:+UseZGC", "-XX:AllocatePrefetchStyle=1", "-XX:+ZGenerational", "-XX:ZMarkStackSpaceLimit=16G"]  # brucethemoose (incompatible with -ZProactive; Java 21+)  # java error; default: 8G  # brucethemoose: 1; default: 1  #481 (line in Coconut source)
